...
// values for base
    ...
    public static class TensorFlowPractice extends LinearOpMode {
     ...
     //values for arm
        ...
        telemetry.update();
            ...
            // directions and measurements are based off red alliance side, placed near warehouse
            if (opModeIsActive()) {
                while (opModeIsActive()) {
            ...
            // directions and measurements are based off blue alliance side, placed near warehouse
            if (opModeIsActive()) {
                while (opModeIsActive()) {
                     // close claw to pinch block
                     claw.setPosition(0);
                    ...
                    if(isDuckOrCubeDetected() == true){ //If Duck is in middle marker (Level 2)
                        ...
                        if(isDuckOrCubeDetected() == true){ //If Duck is in LEVEL 1
                            ...
                            telemetry.addLine("Duck assumed @ LVL 3, Strafing now");
                            telemetry.update();
                            //Strafe right to shipping hub (same distance as Level 1)
                            encoderDriveStrafe(DRIVE_SPEED, -20, -20, 12.0);
